...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * 可以看到，在任何一个构造方法中，都没有对存储Map元素Node的table变量进行初始化。
     * 而是在第一次put操作的时候在进行初始化。
     */
             ...
             */
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {  // 再次取出要存储的位置的元素，跟前面取出来的比较
                        if (fh >= 0) {         // 取出来的元素的hash值大于0，当转换为树之后，hash值为-2, fh > 0 表示为链表，将该节点插入到链表尾部
                            binCount = 1;
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        for (int i = 0, bound = 0;;) {
            ...
            // f.hash == -1 表示遍历到了ForwardingNode节点，意味着该节点已经处理过
                // 这里是控制并发扩容的核心
            else if ((fh = f.hash) == MOVED)
